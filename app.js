...
function workChatText(){let out=`MARVEL CHAMPIONS THEMATIC TWO-HERO MATCHUPS\nCanonical sequence: ${DATA.rules.scope}\nRepeat marker: ${DATA.rules.repeatMarker} means the hero appeared earlier.\nConstraint: ${DATA.rules.sequenceNote}\n\n`;for(const c of DATA.campaigns){out+=`## ${c.campaign}\n`;if(c.note)out+=`Note: ${c.note}\n`;for(const s of c.scenarios){const vs=s.villains.map(x=>DATA.characters[x].display).join(' + ');const hs=s.heroAssignments.map(a=>DATA.characters[a.id].display+(a.repeat?' 🔁':'')).join(' + ');out+=`${s.number}. ${s.title}\nVillain(s): ${vs}\nHeroes: ${hs}\nContext: ${s.context}\n\n`}}out+='Use this sequence as the source of truth. Preserve order, pairings, release gating, and repeat markers unless explicitly asked to revise them.';return out}
