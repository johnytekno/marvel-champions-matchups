...
function heroUsageRows(){const counts=new Map();for(const campaign of DATA.campaigns)for(const scenario of campaign.scenarios)for(const id of scenario.heroes)counts.set(id,(counts.get(id)||0)+1);return [...counts].map(([id,appearances])=>({id,name:DATA.characters[id].display,appearances,repeats:appearances-1})).sort((a,b)=>b.appearances-a.appearances||a.name.localeCompare(b.name))}
function renderHeroUsage(){const rows=heroUsageRows(),repeatTotal=rows.reduce((sum,row)=>sum+row.repeats,0);$('#heroUsageSummary').textContent=`${rows.length} heroes · ${repeatTotal} repeat appearances`;$('#heroUsage').innerHTML=rows.map(row=>`<div class="hero-usage-item"><span class="hero-usage-name" title="${row.name}">${row.name}</span><span class="hero-usage-count">${row.appearances}× · ${row.repeats} repeat${row.repeats===1?'':'s'}</span></div>`).join('')}
